...
function draw() {
  ...
  if (isRight) {
    if (gameChar_x < width * 0.4) {
      ...
      cameraPosX += 5; // Scroll the world when reaching screen edge
    }
  }
...
function drawMountains() {
  ...
  for (var i = 0; i < 3; i++) {
    let y = startY;
    let x1 = startX + cameraPosX / 10 + (mountainHeight - i) * baseWidth;
    let x2 =
      startX + cameraPosX / 6 + ((mountainHeight + i - 10) * baseWidth) / 4;
...
function drawTree() {
  for (var i = 0; i < treePos_X.length; i++) {
    ...
    ellipse(treePos_X[i], treePos_y - 110, 120, 70);
    ellipse(treePos_X[i], treePos_y - 140, 80, 50);
  }
}
...
function drawClouds() {
  for (var i = 0; i < clouds.length; i++) {
    fill(255);
    ellipse(clouds[i].x_pos + cameraPosX / 20, clouds[i].y_pos, 55, 55);
    ellipse(clouds[i].x_pos + 25 + cameraPosX / 20, clouds[i].y_pos, 55, 75);
    ellipse(clouds[i].x_pos + 45 + cameraPosX / 20, clouds[i].y_pos, 55, 55);
  }
}
